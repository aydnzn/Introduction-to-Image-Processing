...
% Feature Extraction: Different sets of features are loaded from MAT files. This includes GIST features and color features. The loaded features are stored in respective variables.
...
% Feature Concatenation: The normalized GIST features and the first 5409 rows of normalized color features are concatenated horizontally to form the features_end matrix. This matrix represents the combined set of features used as input for the SVM model.
% Training and Testing the SVM Model for Safe Classification: An SVM model is trained using the first 2000 instances of the features_end matrix and the corresponding binary safe scores. The fitcsvm function is used to train the SVM model. After training, the model is tested on instances 2001 to 2100 of features_end. The predicted labels and scores are obtained using the predict function. The accuracy of the model is calculated by comparing the predicted labels with the actual binary safe scores.
% Training and Testing the SVM Model for Wealthy Classification: Similar to the safe classification, an SVM model is trained using the first 2000 instances of features_end and the corresponding binary wealthy scores. The model is then tested on instances 2001 to 2100, and the accuracy is calculated.
...
color_features_read_matrix = matfile('color_features_9710_192.mat');
colour_features_end = color_features_read_matrix.features_end;
...
features_end = [];
features_end = [ normGist,normColor(1:5409,:) ]; %, gist_features_end ];
