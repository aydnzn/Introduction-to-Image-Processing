...
% Feature Extraction: Different sets of features are loaded from MAT files. This includes wavelet features and GIST features. The loaded features are stored in respective variables.
% Feature Normalization: The wavelet features and GIST features are individually normalized using min-max normalization. The minimum and maximum values for each feature set are calculated, and then the normalization is performed using the formula (x - min(x)) / (max(x) - min(x)). The normalized features are stored in respective variables.
% Feature Concatenation: The normalized wavelet features and the first 1200 rows of normalized GIST features are concatenated horizontally to form the features_end matrix. This matrix represents the combined set of features used as input for the SVM model.
...
features_end = [];
features_end = [ normWave,normGist(1:1200,:) ]; %, gist_features_end ];
